...
public OnPluginStart()
{
	...
	g_iCredits[17] = KvGetNum(kv,"return2");
	g_iCredits[18] = KvGetNum(kv,"ticket");
	KvRewind(kv);
...
public Select_PL(Handle:menu, MenuAction:action, iClient, option)
{
	if (action == MenuAction_Select)
	{
		...
		if (target)
		{
			...
			CGOPrintToChat(iClient, "{GREEN}Вы дали Игрок {BLUE}%N {GREEN}подкрутку в !lot", target);
		}
		else 
		{
...
public g_LotMenu2_CallBack(Handle:panel, MenuAction:action, client, item)
{
	if (action == MenuAction_Select) {
		if (item == 1) {
			if (Shop_GetClientCredits(client) > g_iCredits[16] - 1)
			{
				//Store_SetClientCredits(client, Store_GetClientCredits(client) - 1500);
				Shop_TakeClientCredits(client, g_iCredits[16]);
				iEx_Start(client);
				decl String:steamid[28];
				if (GetClientAuthId(client, AuthId_Steam2, steamid, sizeof(steamid)))
					SetTrieValue(g_LotHistory_Trie2, steamid, GetTime());
			} else
				PrintToChat(client," \x04[Lot]\x02 У вас не хватает %d кредитов на билет!", g_iCredits[16] - Shop_GetClientCredits(client));
		} else
			if (item == 3)
			{
				PrintToChat(client, " \x04[Lot]\x02 000 -> Шанс Проиграть 2000 Кр.");
				...
				PrintToChat(client, " \x04[Lot]\x02 999 -> Редкий приз! Вам выпадет 4000 Кр.");
				PrintToChat(client, " \x04[Lot]\x02 1000-1100 -> Шанс Проиграть 200 Кр.");
				PrintToChat(client, " \x04[Lot]\x02 1100-1200 -> Шанс, что Вам выпадет 1000 Кр.");
				PrintToChat(client, " \x04[Lot]\x02 1201 -> Супер приз! Шанс, что Вам выпадет 5500 Кр.");
				PrintToChat(client, " \x04[Lot]\x02 1202 -> Самый редкий приз! Шанс, что Вам выпадет 8000 Кр.");
				PrintToChat(client, " \x04[Lot]\x02 1203-1300 -> Шанс, что Вам выпадет 149 Кр.\n \n");
			}
	}
}
...
iEx_Start(client)
{
	...
	new FakeNumber = GetRandomInt(0,1300);
	if(Podkrutka[client]) FakeNumber = 1300;
	new Handle:panel = CreatePanel(); 
	SetPanelTitle(panel, "Ящик Пандоры:\n \n");
