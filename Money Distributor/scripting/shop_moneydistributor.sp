...
{
	CreateConVar("sm_shop_credits_version", PLUGIN_VERSION, "Money distributor version.", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_CHEAT|FCVAR_DONTRECORD);
...
public OnSomeEvent(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (KvGotoFirstSubKey(kv))
	{
		...
		do
		{
			if(KvGetNum(kv, "NoWarmUp", 0) == 1 && GameRules_GetProp("m_bWarmupPeriod") == 1)
				continue;
