...
public Plugin myinfo =
{
	...
	author = "Tonki_Ton & SD",
	version = "1.0.0",
	url = "vk.com/tonki_ton | vk.com/setting_design"
};
			...
			switch(iPick)
			{
				case 0:
				{
					...
					else
					{
						...
						CPrintToChat(iClient, "%sВведите желаемую сумму в чат, %i мин. {red}%i макс.", g_sPluginPrefix, g_iCash[1], g_iCash[0]);
					}
				}
				case 1:
				{
					g_bIsPrinting[1][iClient] = true;
					CPrintToChat(iClient, "%sВведите желаемую сумму в чат.", g_sPluginPrefix);
				}
				case 2:
				{
					InvestMenu(iClient);
		...
		case MenuAction_Select:
		{
			if (!iPick)
			{
				...
				else
				{
					...
					CPrintToChat(iClient, "%sВведите желаемую сумму вклада в чат, %i мин. {red}%i макс.", g_sPluginPrefix, g_iInvestCash_Cash[0], g_iInvestCash_Cash[1]);
				}
			}
			else
			{
				g_bIsPrinting[4][iClient] = true;
				CPrintToChat(iClient, "%sВведите желаемую сумму в чат, {red}%i макс.", g_sPluginPrefix, g_iInvestCash[iClient]);
			}
		}
	}
}
	...
	switch (maAction)
	{
		...
		case MenuAction_Select:
		{
			...
			if (g_bSwitch[iClient])
			{
				...
				CPrintToChat(iClient, "%sВы совершили вклад в банк в размере {red}%i.", g_sPluginPrefix, iAmount);
			...
			else
			{
				...
				else
				{
					...
					CPrintToChat(iClient, "%sВведите количество дней в чат, %i мин. {red}%i макс.", g_sPluginPrefix, g_iDays[0], g_iDays[1]);
				}
			}	
		}
	}
	...
	switch (maAction)
	{
		...
		case MenuAction_Select:
		{
			if (iPick == 1)
			{
				...
				CPrintToChat(iClient, "%sВы взяли кредит на сумму {blue}%i кредитов.", g_sPluginPrefix, g_iTemporaryMoney[iClient]);
				...
				CPrintToChat(iClient, "%sВам необходимо вернуть %i кредитов, краний срок через {blue}%s", g_sPluginPrefix, g_iPayable[iClient], sTime);
...
public Action OnClientSayCommand(int iClient, const char[] command, const char[] sArgs)
{
	if (g_bIsPrinting[0][iClient])    
	{
		...
		{
			CPrintToChat(iClient, "%sНекорректная сумма, %i мин. {red}%i макс.", g_sPluginPrefix, g_iCash[1], g_iCash[0]);
			CPrintToChat(iClient, "%sПовторите попытку.", g_sPluginPrefix);
			return Plugin_Handled;
		}
	...
	else if (g_bIsPrinting[1][iClient])    
	{
		...
		if ((iBet = StringToInt(sArgs)) <= 0 || iBet > g_iPayable[iClient] || iBet > Shop_GetClientCredits(iClient))  
		{
			...
			BankMenu(iClient, 0);
			CPrintToChat(iClient, "%sНекорректная сумма. Сумма не должна привышать сумму вашего долга.", g_sPluginPrefix);
			return Plugin_Handled;
		}
		...
		{
			CPrintToChat(iClient, "%sВы погасили всю сумму долга", g_sPluginPrefix);
			g_iTimeStamp[iClient] = 0;
		}
		else
		{
			CPrintToChat(iClient, "%sВы внесли сумму в размере %i, до погашения задолженности осталось {blue}%i", g_sPluginPrefix, iBet, g_iPayable[iClient]);
		}
	...
	else if (g_bIsPrinting[2][iClient])    
	{
		...
		{
			CPrintToChat(iClient, "%sНеккоректное количество дней, %i мин. {red}%i макс.", g_sPluginPrefix, g_iDays[0], g_iDays[1]);
			CPrintToChat(iClient, "%sПовторите попытку.", g_sPluginPrefix);
			return Plugin_Handled;
		}
	...
	else if (g_bIsPrinting[3][iClient])    
	{
		...
		{
			CPrintToChat(iClient, "%sНекорректная сумма, %i мин. {red}%i макс.", g_sPluginPrefix, g_iInvestCash_Cash[0], g_iInvestCash_Cash[1]);
			CPrintToChat(iClient, "%sПовторите попытку.", g_sPluginPrefix);
			return Plugin_Handled;
		}
		...
		{
			CPrintToChat(iClient, "%sНелья вложить больше {red}%i.", g_sPluginPrefix, g_iMaxAmountInBank);
		...
		CPrintToChat(iClient, "%sВы совершили вклад в банк в размере {red}%i.", g_sPluginPrefix, iBet);
		Shop_TakeClientCredits(iClient, iBet);
		InvestMenu(iClient);
	...
	else if (g_bIsPrinting[4][iClient])    
	{
		...
		{
			CPrintToChat(iClient, "%sНекорректная сумма, {red}%i макс.", g_sPluginPrefix, g_iInvestCash[iClient]);
			CPrintToChat(iClient, "%sПовторите попытку.", g_sPluginPrefix);
			return Plugin_Handled;
		}
		...
		CPrintToChat(iClient, "%sВы забрали из банка вклад в размере {red}%i.", g_sPluginPrefix, iBet);
...
bool CheckDebt(int iClient, int iCredits)
{
	if (iCredits)
	{
		if (iCredits > g_iPayable[iClient])
		{
			...
			CPrintToChat(iClient, "%sУ вас были изъяты кредиты и сумма задолженности была погашена.", g_sPluginPrefix);
		}
		else
		{
			g_iPayable[iClient] -= iCredits;
			Shop_SetClientCredits(iClient, 0);
			CPrintToChat(iClient, "%sУ вас были изъяты имеющиеся кредиты, осталось погасить {blue}%i кредиты.", g_sPluginPrefix, g_iPayable[iClient]);
		}
