...
{
	name = "[Shop] CS:GO Human Skins for Zombie:Reloaded",
	author = "FrozDark Feat R1KO, Tonki_Ton, Oylsister",
...
public void OnMapStart()
{
	...
	Shop_GetCfgFile(buffer, sizeof(buffer), "zr_skins_downloads.txt");
...
public void Shop_Started()
{
	...
	Shop_GetCfgFile(_buffer, sizeof(_buffer), "zr_skins.txt");

	if (kv != INVALID_HANDLE) delete kv;

	kv = CreateKeyValues("ZR_Skins");
...
public ShopAction OnEquipItem(int client, CategoryId category_id, const char[] category, ItemId item_id, const char[] item, bool isOn, bool elapsed)
{
	...
	{
		CPrintToChat(client, "\x04[Shop]\x01 Your skins will be changed in the next round.");
