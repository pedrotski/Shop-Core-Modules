...
public void Shop_Started()
{
	...
	if (!hKeyValues.ImportFromFile(sBuffer)) SetFailState("Could not open file '%s'", sBuffer);
...
public ShopAction OnItemUsed(int iClient, CategoryId category_id, const char[] category, ItemId item_id, const char[] item, bool isOn, bool elapsed)
{
	...
	if(index == PREFIX)
	{
		...
		if(strcmp(g_sClientPrefix[iClient], "custom") == 0)
		{
			...
			PrintToChat(iClient, " \x04[SHOP] \x01x01 To change / set the prefix, enter !tag \"Your prefix\"");
		}
	}
	else
	{
	...
	if(iClient) 
	{
		if(g_bClientPrefix[iClient]) 
		{
			...
			if(sBuffer[0])
			{
				...
				PrintToChat(iClient, " \x04[SHOP] \x01You have set yourself a prefix \"%s\".", sBuffer);
			}
		}
		else
		{
			PrintToChat(iClient, " \x04[SHOP] \x02To use a prefix, purchase it in the store!");
		}
	}
