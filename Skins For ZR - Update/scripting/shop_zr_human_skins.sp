...
public Action Process(Handle timer, any client)
{
	...
	if (!client || selected_id[client] == INVALID_ITEM || IsFakeClient(client) || !IsPlayerAlive(client) || GetClientTeam(client) == CS_TEAM_T) return;
...
public Action ProcessPlayer(Handle timer, any client)
{
	...
	if(GetClientTeam(client) == CS_TEAM_T) return Plugin_Stop;
